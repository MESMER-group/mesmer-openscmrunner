...
def test_create_realisations_from_pre_run_data(test_data_dir):
    mesmer_bundle_file = os.path.join(test_data_dir, "mesmer-bundles", "test-generic-mesmer-bundle.pkl")

    magicc_output_dir = os.path.join(test_data_dir, "magicc-rcmip-phase-1-output")
    magicc_output_gsat = os.path.join(test_data_dir, magicc_output_dir, "rcmip-phase-1_magicc7.1.0.beta-canesm5-r1i1p1f1_world_surface-air-temperature-change.csv")
    magicc_output_ohc = os.path.join(test_data_dir, magicc_output_dir, "rcmip-phase-1_magicc7.1.0.beta-canesm5-r1i1p1f1_world_heat-content-ocean.csv")

    run_years = range(1850, 2100 + 1)
    scenarios_to_run = ["ssp126"]

    openscm_gsat = scmdata.ScmRun(magicc_output_gsat).filter(scenario=scenarios_to_run, year=run_years)
    ...
    openscm_hfds = (
        openscm_ohc
        .delta_per_delta_time(out_var="Heat Uptake|Ocean")
        .convert_unit("W")
